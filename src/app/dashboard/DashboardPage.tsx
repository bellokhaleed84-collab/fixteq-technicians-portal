...
export default function DashboardPage() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-concrete flex items-center justify-center p-4">
        <div className="text-center">
          <p className="text-sm text-steel mb-4">You need to sign in to view your dashboard.</p>
...
function RejectedState() {
  return (
    <div className="min-h-screen bg-ink flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-panel rounded-lg p-8 text-center">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-red-100">
          <span className="text-red-600 text-2xl">✕</span>
        </div>
        <h1 className="font-display font-semibold text-xl text-ink mb-2">Application not approved</h1>
        ...
        <Link
          href="#"
          className="inline-block rounded-md bg-ink px-6 py-3 text-sm font-semibold text-white hover:bg-ink/90 transition-colors"
        >
          Contact Support
        </Link>
      </div>
    ...
    <div className="min-h-screen bg-ink flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-panel rounded-lg p-8 text-center">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full border-4 border-signal/40 bg-signal/10">
            <span className="text-signal text-2xl animate-pulse">⏳</span>
          </div>

          <h1 className="font-display font-semibold text-xl text-ink">Thanks, {firstName}</h1>
          <p className="text-steel text-sm mt-2 mb-6 leading-relaxed">
            Your application is under review. This usually takes 24–48 hours. We&apos;ll notify
            you the moment it&apos;s ready.
          </p>

          <div className="text-left rounded-md border border-steel/15 bg-concrete p-4 space-y-2.5">
            {CHECKLIST.map((item) => (
...
function ApprovedDashboard({
  ...
  return (
    <div className="min-h-screen bg-concrete">
      <header className="sticky top-0 z-10 bg-panel border-b border-steel/10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          <span className="font-display font-semibold text-xl text-ink tracking-tight">Crafteey</span>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-sm font-semibold text-ink leading-tight">{firstName}</p>
              <p className="text-xs text-steel leading-tight">{tradeLabel}</p>
            </div>
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-ink text-white text-sm font-bold">
              {firstName.charAt(0)}
            </div>
          </div>
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 pb-28 space-y-6">
        {page === "queue" &&
          (jobsLoading ? (
            <p className="text-sm text-steel text-center py-10">Loading jobs...</p>
          ) : (
            <QueuePage
              isOnline={isOnline}
...
function BottomNav({
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-10 bg-panel border-t border-steel/10 pb-[env(safe-area-inset-bottom)]">
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {!!item.badge && (
                <span className="absolute top-1.5 right-1/2 translate-x-4 rounded-full bg-signal px-1.5 py-0.5 text-[9px] font-bold text-ink">
                  {item.badge}
                </span>
              )}
            </button>
...
function QueueIcon({ active }: { active: boolean }) {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? "#15181F" : "#3A4250"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function BackIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#15181F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M19 12H5" />
...
function QueuePage({
  ...
  return (
    <>
      <div className="rounded-md border border-steel/15 bg-panel p-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span
            className={`h-2.5 w-2.5 rounded-full ${isOnline ? "bg-emerald-500" : "bg-steel/40"}`}
          />
          <div>
        ...
        <button
          ...
          className={`relative h-7 w-12 shrink-0 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-signal/40 ${
            isOnline ? "bg-emerald-500" : "bg-steel/20"
          }`}
        >
          <span
      ...
      <div className="flex gap-1 rounded-md bg-steel/10 p-1">
  ...
  return (
    <button
      ...
      className={`flex-1 rounded px-3 py-2 text-sm font-semibold transition-colors ${
        active ? "bg-panel text-ink shadow-sm" : "text-steel hover:text-ink"
      }`}
    >
      {children}
    </button>
...
function EmptyState({ title, body }: { title: string; body: string }) {
  return (
    <div className="rounded-md border border-dashed border-steel/25 bg-panel p-8 text-center">
      <p className="text-sm font-semibold text-ink">{title}</p>
      <p className="text-xs text-steel mt-1">{body}</p>
    </div>
  );
}
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-steel/15 bg-panel p-4">
...
function JobCard({ job, onSelect }: { job: Job; onSelect: (id: string) => void }) {
  return (
    <button
      ...
      className={`w-full text-left rounded-md border border-steel/15 border-l-4 ${STATUS_BORDER[job.status]} bg-panel p-4 hover:border-steel/30 transition-colors`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-ink">{job.clientName}</p>
          <p className="text-xs text-steel">{job.address}</p>
        </div>
        <span
          className={`shrink-0 rounded px-2.5 py-1 text-[11px] font-semibold ${STATUS_STYLES[job.status]}`}
        >
          {STATUS_LABEL[job.status]}
        </span>
      </div>

      <p className="text-sm text-steel mt-3">{job.description}</p>

      <div className="flex items-center justify-between mt-3 text-xs text-steel">
        <span>{job.status === "completed" ? `Completed ${job.completedAt}` : job.scheduledFor}</span>
        <span className="font-bold text-ink">{formatNaira(job.price)}</span>
      </div>
    </button>
  );
}
      ...
      <header className="sticky top-0 z-10 bg-panel border-b border-steel/10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          <button
            ...
            onClick={onBack}
            aria-label="Back"
            className="flex h-9 w-9 items-center justify-center rounded-full hover:bg-steel/10 transition-colors -ml-1.5"
          >
            <BackIcon />
          </button>
          <h1 className="font-display text-base font-semibold text-ink">Job details</h1>
        </div>
      </header>

      <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        <div className={`rounded-md border border-steel/15 border-l-4 ${STATUS_BORDER[job.status]} bg-panel p-5`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-base font-bold text-ink">{job.clientName}</p>
              <p className="text-xs text-steel mt-0.5">{job.category}</p>
            </div>
            <span
              className={`shrink-0 rounded px-2.5 py-1 text-[11px] font-semibold ${STATUS_STYLES[job.status]}`}
            >
          ...
          <p className="text-sm text-steel mt-4">{job.description}</p>

          <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-steel/10">
            <div>
              <p className="text-[11px] font-semibold text-steel">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-ink mt-0.5">{formatNaira(job.price)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-md border border-steel/15 bg-panel p-5">
          ...
          <div className="flex items-start gap-2 text-ink">
            <span className="text-steel mt-0.5">
              <PinIcon />
            </span>
          ...
          <a
            ...
            className="inline-block mt-3 text-sm font-semibold text-ink underline underline-offset-2"
          >
            Get directions
          </a>
        </div>
        ...
        <div className="rounded-md border border-steel/15 bg-panel p-5">
          ...
          <div className="flex gap-2">
            <a
              ...
              className="flex-1 flex items-center justify-center gap-2 rounded-md border border-steel/25 px-3 py-2.5 text-sm font-semibold text-ink hover:bg-steel/5 transition-colors"
            >
              <PhoneIcon />
              Call
            </a>
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 rounded-md border border-steel/15 px-3 py-2.5 text-sm font-semibold text-steel/50 cursor-not-allowed"
            >
              <MessageIcon />
              Message
            </button>
        ...
        {job.status === "new" && (
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 rounded-md border border-steel/25 px-3 py-3 text-sm font-semibold text-ink hover:bg-steel/5 transition-colors"
            >
              Decline
            </button>
            <button
              ...
              className="flex-1 rounded-md bg-ink px-3 py-3 text-sm font-semibold text-white hover:bg-ink/90 transition-colors"
            >
              Accept
            </button>
          </div>
        ...
        {job.status === "accepted" && (
          <button
            ...
            className="w-full rounded-md bg-emerald-500 px-3 py-3 text-sm font-semibold text-white hover:bg-emerald-600 transition-colors"
          >
            Mark as complete
          </button>
        )}
